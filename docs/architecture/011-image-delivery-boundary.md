...
De route is bewust extensieloos: Scryfalls gedocumenteerde `normal`-representatie
is JPEG en de Worker retourneert daarom echte JPEG-bytes met `image/jpeg`.
...
credentials en poort worden opnieuw gecontroleerd. Redirects, non-JPEG-responses,
...
moeten non-success en `no-store` zijn. Een tijdelijke upstreamfout verifieer je
in staging of een gecontroleerde testdeployment; hij mag nooit een latere geldige
response maskeren of een lange cacheheader krijgen.
